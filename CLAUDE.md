...
- GUI startup: the main window is configured `visible: false`, so the first frame the user sees is a painted one — previously the native window appeared before WebKit's first frame, which read as a white flash (issue #20). After applying the theme and rendering the loading UI, the frontend calls `main_window_ready`, which matches the native background to the CSS `--bg` and reveals the window; Dock/popover reopen paths are gated on the same `MainWindowReady` flag so they can't put the empty webview on screen first. Because nothing but that call reveals the window, two fallbacks keep a hidden-forever window off the table: the command parses the colour best-effort (an unparseable `--bg` still reveals) and `setup` reveals unconditionally after 3s if the webview never reported ready. `reveal_main` is idempotent via an atomic swap, so the fallback can't re-show a window the user has since closed. `StateFlags` deliberately omits `VISIBLE`, so window-state restore doesn't fight the hidden start. Animation-frame waits happen only after showing, with a timeout for hidden/minimized windows; `get_snapshot` runs through Tauri's async command path so scanning doesn't block the native event loop while the loading screen is trying to paint.
...
- [ ] Launch cold: the window's first visible frame is the spinner + "Scanning applications…" on the themed background, never a white or empty pane — check in both light and dark
...
- [ ] Keyboard: ⌘F → type → ↑↓ moves the cursor → ↵ opens the sheet; ↑↓/↵ inside the sheet applies (verify with `openwith current <ext>`); Esc clears the search before releasing focus; with the pointer parked on the list, three ↓ move exactly three rows and the hovered row stays dark until the mouse really moves; the popover does the same for matches and its picker
- [ ] Menu-bar icon sits at roughly the same visual weight as neighbouring status items (it renders ~22 x 12pt; `tray-icon` fixes the canvas at 18pt tall and derives width from the canvas aspect)
